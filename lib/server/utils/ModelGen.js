...
class ModelGen {
   ...
   */
  getSchema(fields) {
    const typeMap = FIELD_TYPES.STANDARD.reduce(
      (types, type) => _.assign(types, {
        [type.key]: type.primitiveType
      }),
      {}
    );
  ...
  generateModel(name='Model', schema={}, extensions={}, settings={}) {
    ...
    const Model = connection.useDb(dbName).model(name, ModelSchema);
