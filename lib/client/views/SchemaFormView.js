...
const SchemaFormMutations = gql`
  mutation SchemaFormMutations($_id: ID, $name: String!, $fields: [FieldInput!]!, $description: String) {
    collection: updateOrCreateCollection(_id: $_id, name: $name, fields: $fields, description: $description) {
      _id _db _collection
...
function getFormHandlers() {
  return {
    ...
    submitForm(event) {
      ...
      this.props.updateOrCreateCollection(this.state.collection)
        .then(({ data }) => data.collection)
        .then(User.updateLibrary);
    }
  };
}
