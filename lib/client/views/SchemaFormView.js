...
import 'lib/client/styles/SchemaFormView.less';
...
const SchemaFormMutations = gql`
  mutation SchemaFormMutations($_id: ID, $name: String!, $fields: [FieldInput!]!, $description: String, $icon: String) {
    collection: updateOrCreateCollection(_id: $_id, name: $name, fields: $fields, description: $description, icon: $icon) {
      _id _db _collection
...
const getVariables = ({ _id, name, description, fields, icon, _db = 'test' }) => ({
  _id, name, description, icon, _db, fields: fields.map(
...
class SchemaFormView extends ViewComponent {
  static defaultProps = {
    collection: {
      _id: null,
      name: '',
...
function getFormHandlers() {
  return {
    ...
    submitForm(event) {
      ...
      this.props.updateOrCreateCollection(collection)
        .then(({ data }) => data.collection)
        .then(coll => Promise.resolve(User.updateLibrary(coll)))
        .then(() => this.props.history.push(`/collections${collection.path}`));
    }
  };
}

module.exports = SchemaFormView;
