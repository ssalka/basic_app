...
function getGraphQLComponent(BaseComponent, collectionStore, data = {}) {
  ...
  const storeAPI = api[getGraphQLCollectionType(collection)];
  ...
  if (_document) {
    ...
    @graphql(collectionQuery, {
      ...
      props: ({ data, cursor, ...props }) => (
        _.assign(props, _(data)
          .omit('fetchMore')
          .mapKeys((_, key) => (
            key.replace(collectionName, 'documents')
          ))
          .assign({
