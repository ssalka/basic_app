...
const getVariables = ({ _id, name, description, fields, _db = 'test' }) => ({
  id: _id, name, description, _db,
  fields: fields.map(field => _.omit(field, '__typename'))
});
...
class SchemaForm extends ViewComponent {
  ...
  constructor(props) {
    ...
    if (collection._id) collection.id = collection._id;

    this.state = {
      collection,
      selectingIcon: false,
      editingFields: false,
      showFieldOptions: collection.id
