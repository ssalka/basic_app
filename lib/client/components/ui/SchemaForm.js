...
const SchemaFormMutations = gql`
  mutation SchemaFormMutations($id: ID, $name: String!, $fields: [FieldInput!]!, $description: String) {
    updateOrCreateCollection(id: $id, name: $name, fields: $fields, description: $description) {
      name
      _collection
      description
...
const getVariables = ({ _id, name, description, fields, _db = 'test' }) => ({
  _id, name, description, _db,
  fields: fields.map(field => _.assign(field, {
    type: _.find(FIELD_TYPES, { name }).key
  }))
});
            ...
            <div className="fields">
              {collection.fields.map(({name, type}, index) => (
                <FlexColumn key={index}>
                  ...
                  {showFieldOptions[index] && !editingFields ? <FieldOptions index={index} /> : null}
                </FlexColumn>
              ))}
            </div>
          </div>
